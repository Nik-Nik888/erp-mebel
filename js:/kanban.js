      <div style="display:flex;align-items:center;gap:6px;padding:4px 0;margin-bottom:4px">
        ...
        <span style="font-size:12px;font-weight:600;color:${st.color}">${st.name}</span>
        <span style="font-size:10px;color:var(--text3)">${selectedCount}/${stageWorks.length}</span>
        ${stageTotal?`<span style="font-size:10px;font-weight:600;color:var(--accent-text)">${stageTotal.toLocaleString('ru-RU')} ₽</span>`:''}
      </div>`;
    
    stageWorks.forEach(r=>{
      const wd=getWorkData(r.id);
      const checked=!!wd;
      const qty=wd?wd.qty||1:1;
      const lineTotal=checked?(parseFloat(r.rate)||0)*qty:0;
      h+=`<div style="display:flex;align-items:center;gap:6px;padding:4px 8px;border-radius:var(--rs);font-size:12px;background:${checked?st.color+'08':''}">
        <input type="checkbox" ${checked?'checked':''} onchange="toggleOrderWork('${st.key}',${r.id},this.checked)" style="margin:0;flex-shrink:0">
        <span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.work_name}</span>
        <input type="number" value="${qty}" min="0.5" step="0.5" onchange="updateOrderWorkQty('${st.key}',${r.id},this.value)" style="width:45px;border:1px solid var(--border);border-radius:4px;padding:2px 4px;font-size:11px;text-align:center;font-family:'Geologica',sans-serif;outline:none;background:var(--surface)${!checked?';opacity:0.3':''}">
        <span style="font-size:10px;color:var(--text3);min-width:45px">${r.unit||''}</span>
        <span style="font-size:10px;color:var(--text3);min-width:35px;text-align:right">×${(parseFloat(r.rate)||0)}</span>
        ${checked?`<span style="font-size:11px;font-weight:500;color:var(--accent-text);min-width:50px;text-align:right">${lineTotal.toLocaleString('ru-RU')}₽</span>`:''}
      </div>`;
    });
    h+=`</div>`;
  });
  ...
  if(!hasAnyWorks){
    h+='<div style="padding:12px;text-align:center;color:var(--text3);font-size:12px">Нет видов работ. Добавьте в Зарплата → 💰 Расценки</div>';
  }
  
  body.innerHTML=h;
}

function toggleOrderWork(stageKey,rateId,checked){
  let works={};
  try{works=JSON.parse($('f-works-data').value||'{}')}catch(e){}
  
  if(!works[stageKey]) works[stageKey]=[];
  // Нормализуем в новый формат [{id,qty}]
  works[stageKey]=works[stageKey].map(w=>typeof w==='object'?w:{id:w,qty:1});
  
  if(checked){
    if(!works[stageKey].find(w=>w.id===rateId)) works[stageKey].push({id:rateId,qty:1});
  } else {
    works[stageKey]=works[stageKey].filter(w=>w.id!==rateId);
  }
  if(!works[stageKey].length) delete works[stageKey];
  
  $('f-works-data').value=JSON.stringify(works);
  updateWorksCount(works);
  if(editId) saveWorksNow(works);
  // Перерендерим
  const stages={};try{Object.assign(stages,JSON.parse($('f-stages-data').value||'{}'))}catch(e){}
  renderWorksForm(works,stages);
}

function updateOrderWorkQty(stageKey,rateId,qty){
  let works={};
  try{works=JSON.parse($('f-works-data').value||'{}')}catch(e){}
  if(!works[stageKey]) works[stageKey]=[];
  works[stageKey]=works[stageKey].map(w=>typeof w==='object'?w:{id:w,qty:1});
  
  const item=works[stageKey].find(w=>w.id===rateId);
  if(item) item.qty=parseFloat(qty)||1;
  else works[stageKey].push({id:rateId,qty:parseFloat(qty)||1});
...
function updateWorksCount(works){
  let total=0;
  Object.values(works).forEach(arr=>{if(Array.isArray(arr)) total+=arr.length});
  const badge=$('f-works-count');
  if(badge) badge.textContent=total;
}

function toggleAllWorks(stageKey,selectAll){
  let works={};
  try{works=JSON.parse($('f-works-data').value||'{}')}catch(e){}
  
  if(selectAll){
    works[stageKey]=payRates.filter(r=>r.stage_key===stageKey).map(r=>({id:r.id,qty:1}));
  } else {
    delete works[stageKey];
  }
...
async function saveWorksNow(works){
  if(!editId) return;
  try{
    await sb.from('orders').update({works}).eq('id',editId);
  }catch(e){console.log('Works save error:',e)}
}
